...
function renderTable(data) {
    ...
    tbody.innerHTML = data.map(row => `
        <tr>
            ...
            <td style="color:blue; font-weight:bold;">${row.jar_daily.toFixed(1)}</td>
            <td>${Math.round(row.keg_price)}</td>
            <td>${Math.round(row.keg_net)}</td>
            <td style="color:blue; font-weight:bold;">${row.keg_daily.toFixed(1)}</td>
            <td>${row.type === 'fruit' ? Math.round(row.dehyd_price) : '-'}</td>
            <td>${row.type === 'fruit' ? Math.round(row.dehyd_net) : '-'}</td>
            <td style="color:blue; font-weight:bold;">${row.type === 'fruit' ? row.dehyd_daily.toFixed(1) : '-'}</td>
        </tr>
    `).join('');
}
...
// 使用 DOMContentLoaded 比 window.onload 更穩定
